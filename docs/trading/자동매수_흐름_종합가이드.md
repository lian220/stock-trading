# 🤖 자동 매수 시스템 종합 가이드

자동 매수 시스템의 전체 흐름을 분석하고, 그 흐름에 맞게 코딩을 하게 하는 종합 가이드입니다.

## 📋 목차

1. [시스템 아키텍처 개요](#시스템-아키텍처-개요)
2. [자동 매수 전체 흐름](#자동-매수-전체-흐름)
3. [단계별 상세 분석](#단계별-상세-분석)
4. [코드 구조 및 파일 위치](#코드-구조-및-파일-위치)
5. [데이터 흐름 다이어그램](#데이터-흐름-다이어그램)
6. [코딩 가이드라인](#코딩-가이드라인)
7. [설정 및 커스터마이징](#설정-및-커스터마이징)
8. [API 엔드포인트 정리](#api-엔드포인트-정리)
9. [에러 처리 및 로깅](#에러-처리-및-로깅)
10. [실전 활용 예시](#실전-활용-예시)

---

## 시스템 아키텍처 개요

### 🏗️ 전체 구조

```
┌─────────────────────────────────────────────────────────────┐
│                    자동 매수 시스템 아키텍처                    │
└─────────────────────────────────────────────────────────────┘

┌──────────────┐      ┌──────────────┐      ┌──────────────┐
│   스케줄러    │─────▶│  자동매매     │─────▶│  주문 실행    │
│  (Scheduler)  │      │   서비스     │      │  (KIS API)   │
└──────────────┘      └──────────────┘      └──────────────┘
       │                     │                       │
       │                     │                       │
       ▼                     ▼                       ▼
┌──────────────┐      ┌──────────────┐      ┌──────────────┐
│  추천 서비스  │      │  설정 관리    │      │  잔고 조회    │
│ (Recommend)  │      │  (Config)     │      │  (Balance)   │
└──────────────┘      └──────────────┘      └──────────────┘
       │
       │
       ▼
┌─────────────────────────────────────────────────────────────┐
│                    데이터 소스 레이어                          │
├─────────────────────────────────────────────────────────────┤
│  Supabase: stock_analysis_results, stock_recommendations,     │
│            ticker_sentiment_analysis, auto_trading_config    │
│  MongoDB:  daily_stock_data, stocks, users                   │
└─────────────────────────────────────────────────────────────┘
```

### 📁 주요 컴포넌트

| 컴포넌트 | 파일 위치 | 역할 |
|---------|----------|------|
| **스케줄러** | `app/utils/scheduler.py` | 매일 밤 12시 자동 매수 트리거 |
| **자동매매 서비스** | `app/services/auto_trading_service.py` | 매수 로직 실행 및 관리 |
| **추천 서비스** | `app/services/stock_recommendation_service.py` | 매수 대상 선정 |
| **잔고 서비스** | `app/services/balance_service.py` | 주문 실행 및 잔고 조회 |
| **API 라우터** | `app/api/routes/auto_trading.py` | REST API 엔드포인트 |
| **스키마** | `app/schemas/auto_trading.py` | 데이터 모델 정의 |

---

## 자동 매수 전체 흐름

### 🔄 전체 프로세스 플로우

```
┌─────────────────────────────────────────────────────────────┐
│ 1. 스케줄러 트리거 (매일 밤 12시 한국 시간)                    │
└───────────────────────┬─────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────────┐
│ 2. 실행 조건 확인                                             │
│    ├─ 주말 체크 (뉴욕 시간 기준)                              │
│    ├─ 미국 장 시간 확인 (9:30 AM - 4:00 PM ET)               │
│    └─ 자동매매 활성화 여부 확인                               │
└───────────────────────┬─────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────────┐
│ 3. 매수 후보 선정                                             │
│    ├─ 통합 추천 조회 (AI + 기술적 지표 + 감정 분석)            │
│    ├─ 종합 점수 필터링 (min_composite_score)                 │
│    ├─ 감정 점수 필터링 (min_sentiment_score)                  │
│    ├─ 최대 매수 종목 수 제한 (max_stocks_to_buy)              │
│    └─ 종합 점수 기준 정렬 (높은 순)                           │
└───────────────────────┬─────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────────┐
│ 4. 보유 종목 확인                                             │
│    └─ 중복 매수 방지 (선택적)                                 │
└───────────────────────┬─────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────────┐
│ 5. 각 종목별 매수 처리                                        │
│    ├─ 레버리지 설정 확인 (MongoDB users 컬렉션)               │
│    ├─ 거래소 코드 결정 (NASD/NYSE)                             │
│    ├─ 현재가 조회 (여러 거래소 시도)                           │
│    ├─ 매수 가능 수량 계산 (inquire_psamount)                  │
│    ├─ 매수 주문 실행 (order_overseas_stock)                   │
│    ├─ 주문 기록 저장 (auto_trading_logs)                      │
│    └─ Slack 알림 전송                                         │
└───────────────────────┬─────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────────┐
│ 6. 완료 및 로깅                                               │
│    ├─ 성공/실패 통계                                          │
│    ├─ 남은 잔고 정보                                          │
│    └─ 로그 파일 기록                                          │
└───────────────────────┬─────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────────┐
│ 7. 체결 확인 (비동기)                                         │
│    ├─ 주문 접수 후 5초 대기                                    │
│    ├─ 주문체결내역 API로 체결 여부 확인                        │
│    ├─ 체결 시 상태 업데이트 및 Slack 알림                      │
│    └─ 미체결 시 pending 상태로 유지                           │
└───────────────────────┬─────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────────┐
│ 8. 장 마감 후 미체결 주문 정리 (매일 06:30)                    │
│    ├─ pending/accepted 상태 주문 조회                         │
│    ├─ failed 상태로 변경                                      │
│    └─ Slack 알림 전송                                         │
└─────────────────────────────────────────────────────────────┘
```

---

## 단계별 상세 분석

### 1️⃣ 스케줄러 트리거

**파일**: `app/utils/scheduler.py`

**코드 위치**:
```python
# 스케줄러 시작
schedule.every().day.at("00:00").do(self._run_auto_buy)

# 실행 함수
def _run_auto_buy(self, send_slack_notification: bool = True):
    """자동 매수 실행 함수 - 스케줄링된 시간에 실행됨"""
    # 중복 실행 방지
    if self.buy_executing:
        return False
    
    self.buy_executing = True
    # 주말 체크
    # 비동기 실행
```

**핵심 로직**:
- 매일 밤 12시 (한국 시간) 자동 실행
- 중복 실행 방지 플래그 (`buy_executing`)
- 주말 체크 (뉴욕 시간 기준)
- 비동기 실행을 위한 스레드 생성

**코딩 가이드**:
```python
# ✅ 올바른 예시
def _run_auto_buy(self, send_slack_notification: bool = True):
    if self.buy_executing:  # 중복 실행 방지
        return False
    
    self.buy_executing = True
    try:
        # 주말 체크
        now_ny = datetime.now(pytz.timezone('America/New_York'))
        if now_ny.weekday() >= 5:  # 주말
            return False
        
        # 비동기 실행
        thread = threading.Thread(target=run_in_thread)
        thread.start()
    finally:
        self.buy_executing = False
```

---

### 2️⃣ 실행 조건 확인

**파일**: `app/utils/scheduler.py` → `_execute_auto_buy()`

**코드 위치**:
```python
async def _execute_auto_buy(self, send_slack_notification: bool = True):
    # 미국 장 시간 확인
    now_in_ny = datetime.now(pytz.timezone('America/New_York'))
    ny_hour = now_in_ny.hour
    ny_minute = now_in_ny.minute
    ny_weekday = now_in_ny.weekday()
    
    # 장 시간 체크
    is_weekday = 0 <= ny_weekday <= 4
    is_market_open_time = (
        (ny_hour == 9 and ny_minute >= 30) or
        (10 <= ny_hour < 16) or
        (ny_hour == 16 and ny_minute == 0)
    )
    
    if not (is_weekday and is_market_open_time):
        logger.info("미국 장 시간이 아니므로 매수 작업을 건너뜁니다.")
        return
```

**핵심 로직**:
- 뉴욕 시간 기준으로 장 시간 확인
- 서머타임 자동 고려 (`pytz.timezone('America/New_York')`)
- 평일 9:30 AM - 4:00 PM ET만 실행

**코딩 가이드**:
```python
# ✅ 올바른 예시: 서머타임 자동 고려
now_in_ny = datetime.now(pytz.timezone('America/New_York'))

# ❌ 잘못된 예시: 한국 시간 기준
now_in_korea = datetime.now()  # 서머타임 미고려
```

---

### 3️⃣ 매수 후보 선정

**파일**: `app/services/auto_trading_service.py` → `get_buy_candidates()`

**코드 위치**:
```python
def get_buy_candidates(self, config: Optional[Dict] = None) -> List[Dict]:
    # 1. 통합 추천 조회
    recommendations = self.stock_service.get_combined_recommendations_with_technical_and_sentiment(
        send_slack_notification=False
    )
    
    # 2. 필터링
    candidates = []
    for stock in recommendations["results"]:
        # 종합 점수 필터링
        if stock.get("composite_score", 0) < config.get("min_composite_score", 2.0):
            continue
        
        # 감정 분석 필터링
        if config.get("use_sentiment", False):
            sentiment_score = stock.get("sentiment_score")
            if sentiment_score is None or sentiment_score < config.get("min_sentiment_score", 0.15):
                continue
        
        candidates.append(stock)
    
    # 3. 정렬 및 제한
    candidates.sort(key=lambda x: x.get("composite_score", 0), reverse=True)
    candidates = candidates[:config.get("max_stocks_to_buy", 5)]
    
    return candidates
```

**핵심 로직**:
- `get_combined_recommendations_with_technical_and_sentiment()` 호출
- 종합 점수 필터링 (`min_composite_score`)
- 감정 점수 필터링 (`min_sentiment_score`, 선택적)
- 최대 매수 종목 수 제한 (`max_stocks_to_buy`)
- 종합 점수 기준 내림차순 정렬

**데이터 소스**:
- **Supabase**: `stock_analysis_results`, `stock_recommendations`, `ticker_sentiment_analysis`
- **MongoDB**: `daily_stock_data` (기술적 지표 계산용)

**코딩 가이드**:
```python
# ✅ 올바른 예시: 설정 기반 필터링
if stock.get("composite_score", 0) < config.get("min_composite_score", 2.0):
    continue

# ✅ 올바른 예시: 선택적 필터링
if config.get("use_sentiment", False):
    if sentiment_score < config.get("min_sentiment_score", 0.15):
        continue

# ❌ 잘못된 예시: 하드코딩된 값
if stock.get("composite_score", 0) < 2.0:  # 설정 무시
    continue
```

---

### 4️⃣ 보유 종목 확인

**파일**: `app/services/auto_trading_service.py` → `execute_auto_buy()`

**코드 위치**:
```python
# 현재 보유 종목 확인
balance_result = get_overseas_balance()
holding_tickers = set()
if balance_result.get("rt_cd") == "0" and "output1" in balance_result:
    holding_tickers = {item.get("ovrs_pdno") for item in balance_result["output1"]}

# 매수 처리 중
for candidate in candidates:
    ticker = candidate.get("ticker")
    # 이미 보유 중인 종목은 스킵 (선택적)
    # if ticker in holding_tickers:
    #     continue
```

**핵심 로직**:
- `get_overseas_balance()` API 호출
- 보유 종목 티커 추출
- 중복 매수 방지 (현재는 주석 처리됨)

**코딩 가이드**:
```python
# ✅ 올바른 예시: API 응답 검증
if balance_result.get("rt_cd") == "0" and "output1" in balance_result:
    holding_tickers = {item.get("ovrs_pdno") for item in balance_result["output1"]}

# ❌ 잘못된 예시: 응답 검증 없음
holding_tickers = {item.get("ovrs_pdno") for item in balance_result["output1"]}
```

---

### 5️⃣ 각 종목별 매수 처리

**파일**: `app/utils/scheduler.py` → `_execute_auto_buy()`

#### 5-1. 레버리지 설정 확인

**코드 위치**:
```python
# MongoDB에서 사용자별 레버리지 설정 조회
from app.infrastructure.database.mongodb_client import get_mongodb_database
db = get_mongodb_database()
user = db.users.find_one({"user_id": user_id})
if user and user.get("stocks"):
    for stock in user.get("stocks", []):
        ticker = stock.get("ticker")
        if ticker:
            user_leverage_map[ticker] = {
                "use_leverage": stock.get("use_leverage", False),
                "leverage_ticker": stock.get("leverage_ticker")
            }

# 레버리지 적용
actual_ticker = ticker
if ticker in user_leverage_map:
    leverage_info = user_leverage_map[ticker]
    if leverage_info["use_leverage"] and leverage_info["leverage_ticker"]:
        actual_ticker = leverage_info["leverage_ticker"]
```

**핵심 로직**:
- MongoDB `users` 컬렉션에서 사용자 설정 조회
- 레버리지 활성화 시 레버리지 티커로 변경
- 예: `AAPL` → `AAPL3L` (3배 레버리지)

**코딩 가이드**:
```python
# ✅ 올바른 예시: 안전한 조회
user = db.users.find_one({"user_id": user_id})
if user and user.get("stocks"):
    # 처리

# ❌ 잘못된 예시: None 체크 없음
for stock in user.get("stocks", []):  # user가 None이면 에러
    pass
```

#### 5-2. 거래소 코드 결정

**코드 위치**:
```python
# 거래소 코드 결정
if actual_ticker.endswith(".X") or actual_ticker.endswith(".N"):
    exchange_code = "NYSE" if actual_ticker.endswith(".N") else "NASD"
    pure_ticker = actual_ticker.split(".")[0]
else:
    exchange_code = "NASD"  # 기본값
    pure_ticker = actual_ticker

# API 요청용 거래소 코드 변환
api_exchange_code = "NAS"
if exchange_code == "NYSE":
    api_exchange_code = "NYS"
elif exchange_code == "AMEX":
    api_exchange_code = "AMS"
```

**핵심 로직**:
- 티커에 거래소 구분이 포함된 경우 파싱
- 기본값은 NASDAQ (`NASD`)
- API 요청 시 코드 변환 (`NASD` → `NAS`)

**코딩 가이드**:
```python
# ✅ 올바른 예시: 안전한 파싱
if actual_ticker.endswith(".X") or actual_ticker.endswith(".N"):
    exchange_code = "NYSE" if actual_ticker.endswith(".N") else "NASD"
    pure_ticker = actual_ticker.split(".")[0]
else:
    exchange_code = "NASD"
    pure_ticker = actual_ticker

# ❌ 잘못된 예시: 인덱스 에러 가능
pure_ticker = actual_ticker.split(".")[0]  # "."이 없으면 에러
```

#### 5-3. 현재가 조회

**코드 위치**:
```python
# 여러 거래소 시도
exchanges = ["NAS", "AMS", "NYS"]
price_result = None

# 기본 거래소를 맨 앞으로
default_exchange = self._get_exchange_code(ticker).replace("D", "S")
if default_exchange in exchanges:
    exchanges.remove(default_exchange)
    exchanges.insert(0, default_exchange)

for exchange in exchanges:
    temp_result = get_current_price({
        "EXCD": exchange,
        "SYMB": ticker
    })
    
    output = temp_result.get("output", {})
    if temp_result.get("rt_cd") == "0" and (output.get("last") or output.get("base")):
        price_result = temp_result
        break

# 현재가 추출
current_price = float(price_result.get("output", {}).get("last") or 0)

# 대체 가격 사용 (현재가가 0인 경우)
if current_price <= 0:
    base_price = float(price_result.get("output", {}).get("base") or 0)
    if base_price > 0:
        current_price = base_price
    else:
        # 추천 데이터의 가격 사용
        cached_last_price = float(candidate.get("last_price") or 0)
        predicted_price = float(candidate.get("predicted_price") or 0)
        if cached_last_price > 0:
            current_price = cached_last_price
        elif predicted_price > 0:
            current_price = predicted_price
```

**핵심 로직**:
- 여러 거래소에서 현재가 조회 시도
- 실시간 가격(`last`) 우선, 없으면 전일 종가(`base`) 사용
- 모두 실패 시 추천 데이터의 가격 사용

**코딩 가이드**:
```python
# ✅ 올바른 예시: 안전한 타입 변환
try:
    current_price = float(price_result.get("output", {}).get("last") or 0)
    if current_price <= 0:
        # 대체 가격 시도
        base_price = float(price_result.get("output", {}).get("base") or 0)
except (ValueError, TypeError):
    logger.error("가격 변환 실패")
    continue

# ❌ 잘못된 예시: 타입 변환 없음
current_price = price_result.get("output", {}).get("last")  # 문자열일 수 있음
```

#### 5-4. 매수 가능 수량 계산

**코드 위치**:
```python
# 매수 가능 수량 계산
quantity_result = self.calculate_buy_quantity(
    ticker, exchange_code, current_price, max_amount
)

if not quantity_result.get("success") or quantity_result.get("quantity", 0) <= 0:
    logger.warning(f"{ticker} - 매수 가능 수량 없음")
    continue

quantity = quantity_result["quantity"]
```

**파일**: `app/services/auto_trading_service.py` → `calculate_buy_quantity()`

**코드 위치**:
```python
def calculate_buy_quantity(self, ticker: str, exchange_code: str, target_price: float, 
                           max_amount: float) -> Dict:
    # 매수가능금액 조회
    params = {
        "CANO": settings.KIS_CANO,
        "ACNT_PRDT_CD": settings.KIS_ACNT_PRDT_CD,
        "OVRS_EXCG_CD": exchange_code,
        "ITEM_CD": ticker,
        "OVRS_ORD_UNPR": str(target_price)
    }
    
    result = inquire_psamount(params)
    
    if result.get("rt_cd") != "0":
        return {"success": False, "error": result.get("msg1", "조회 실패")}
    
    # 매수 가능 금액 확인
    output = result.get("output", {})
    max_buy_amount = float(output.get("max_ord_psbl_amt") or 0)
    
    # 설정된 최대 금액과 비교하여 더 작은 값 사용
    actual_max_amount = min(max_buy_amount, max_amount)
    
    # 수량 계산
    quantity = int(actual_max_amount / target_price)
    
    return {
        "success": True,
        "quantity": quantity,
        "estimated_amount": quantity * target_price,
        "max_available": max_buy_amount
    }
```

**핵심 로직**:
- `inquire_psamount()` API 호출로 매수 가능 금액 조회
- 설정된 최대 금액(`max_amount_per_stock`)과 비교
- 더 작은 값으로 수량 계산

**코딩 가이드**:
```python
# ✅ 올바른 예시: 안전한 계산
try:
    max_buy_amount = float(output.get("max_ord_psbl_amt") or 0)
except ValueError:
    max_buy_amount = 0.0

actual_max_amount = min(max_buy_amount, max_amount)
quantity = int(actual_max_amount / target_price)

# ❌ 잘못된 예시: 나눗셈 에러 가능
quantity = int(max_buy_amount / target_price)  # target_price가 0이면 에러
```

#### 5-5. 매수 주문 실행

**코드 위치**:
```python
# 실제 주문 실행
order_data = {
    "CANO": settings.KIS_CANO,
    "ACNT_PRDT_CD": settings.KIS_ACNT_PRDT_CD,
    "PDNO": ticker,
    "OVRS_EXCG_CD": exchange_code,
    "ORD_QTY": str(quantity),
    "OVRS_ORD_UNPR": str(current_price),
    "is_buy": True,
    "ORD_DVSN": config.get("order_type", "00")  # 00: 지정가
}

order_result = order_overseas_stock(order_data)

# 주문 기록 저장
self._save_order_log(order_info, "buy")

# API Rate Limit 방지를 위한 대기
time.sleep(0.5)
```

**파일**: `app/services/balance_service.py` → `order_overseas_stock()`

**핵심 로직**:
- `order_overseas_stock()` API 호출
- 주문 구분: `00` (지정가)
- 주문 기록 저장 (`auto_trading_logs` 테이블)
- Slack 알림 전송 (성공/실패 모두)

**코딩 가이드**:
```python
# ✅ 올바른 예시: 주문 데이터 검증
if quantity <= 0 or current_price <= 0:
    logger.error("유효하지 않은 주문 데이터")
    continue

order_data = {
    "ORD_QTY": str(quantity),  # 문자열로 변환
    "OVRS_ORD_UNPR": str(current_price),  # 문자열로 변환
    "is_buy": True
}

# ❌ 잘못된 예시: 타입 변환 없음
order_data = {
    "ORD_QTY": quantity,  # API는 문자열을 요구
    "OVRS_ORD_UNPR": current_price
}
```

---

### 6️⃣ 체결 확인 (비동기)

**파일**: `app/utils/scheduler.py` → `_check_and_update_execution()`

**코드 위치**:
```python
async def _check_and_update_execution(
    self,
    order_no: str,
    ticker: str,
    stock_name: str,
    function_name: str = "_execute_auto_buy",
    before_quantity: int = 0,
    order_quantity: int = 0
):
    # 5초 대기 후 체결 확인
    await asyncio.sleep(5)
    
    # 주문체결내역 API로 체결 여부 확인
    execution_result = check_order_execution(
        order_no=order_no,
        exchange_code=exchange_code,
        ticker=ticker,
        max_retries=3,
        retry_delay=5
    )
    
    if execution_result.get("executed"):
        # 체결 성공 - 상태 업데이트 및 Slack 알림
        db.trading_logs.update_one(
            {"_id": log_record["_id"]},
            {"$set": {"status": "executed", ...}}
        )
    else:
        # 미체결 - pending 상태로 유지
        db.trading_logs.update_one(
            {"_id": log_record["_id"]},
            {"$set": {"status": "pending", ...}}
        )
```

**핵심 로직**:
- 주문 접수 후 5초 대기 (체결까지 시간 필요)
- 주문체결내역 API(`get_overseas_order_detail`)로 체결 여부 확인
- 체결 시: `executed` 상태로 변경, Slack 알림 전송
- 미체결 시: `pending` 상태로 유지, 미체결 알림 전송
- Fallback: 주문 조회 실패 시 잔고 조회로 체결 여부 확인

**체결 확인 방법**:
1. **주문체결내역 API**: 주문번호로 조회하여 `CNTG_YN` 또는 `nccs_qty` 확인
2. **잔고 조회**: 주문 접수 전 보유 수량과 비교하여 증가 여부 확인

**코딩 가이드**:
```python
# ✅ 올바른 예시: 비동기 체결 확인
if order_no:
    asyncio.create_task(self._check_and_update_execution(
        order_no=order_no,
        ticker=ticker,
        stock_name=stock_name,
        before_quantity=before_quantity,
        order_quantity=quantity
    ))

# ❌ 잘못된 예시: 동기 실행 (다음 종목 매수 지연)
if order_no:
    await self._check_and_update_execution(...)  # 다음 종목 매수가 지연됨
```

---

### 7️⃣ 장 마감 후 미체결 주문 정리

**파일**: `app/utils/scheduler.py` → `_cleanup_pending_orders()`

**코드 위치**:
```python
def _cleanup_pending_orders(self, send_slack_notification: bool = True):
    """장 마감 후 미체결 주문 일괄 정리 (실패 상태로 변경)"""
    # 장 마감 확인 (16:00 ET 이후)
    now_in_ny = datetime.now(pytz.timezone('America/New_York'))
    if not is_after_market_close:
        return
    
    # 미체결 주문 조회
    pending_orders = list(db.trading_logs.find({
        "status": {"$in": ["pending", "accepted"]},
        "created_at": {"$gte": yesterday_start}
    }))
    
    # 각 주문을 실패 상태로 변경
    for order in pending_orders:
        db.trading_logs.update_one(
            {"_id": order["_id"]},
            {
                "$set": {
                    "status": "failed",
                    "failed_at": datetime.now(),
                    "failure_reason": "장 마감 후 미체결로 인한 자동 취소"
                }
            }
        )
```

**핵심 로직**:
- 매일 새벽 6시 30분 (한국 시간) 자동 실행
- 장 마감 후(16:00 ET 이후)에만 실행
- `pending` 또는 `accepted` 상태인 주문 조회
- 모든 미체결 주문을 `failed` 상태로 변경
- 개별 및 요약 Slack 알림 전송

**처리 대상**:
- 어제부터 오늘까지 생성된 미체결 주문
- 상태가 `pending` 또는 `accepted`인 주문

**코딩 가이드**:
```python
# ✅ 올바른 예시: 장 마감 확인
is_weekday = 0 <= ny_weekday <= 4
is_after_market_close = ny_hour >= 16

if not is_after_market_close and is_weekday:
    logger.info("장 마감 전입니다. 정리 작업을 건너뜁니다.")
    return

# ❌ 잘못된 예시: 시간 확인 없이 실행
# 장 시간 중에도 실행될 수 있음
```

---

## 코드 구조 및 파일 위치

### 📂 파일 구조

```
app/
├── services/
│   ├── auto_trading_service.py      # 자동매매 서비스 (매수 로직)
│   ├── stock_recommendation_service.py  # 추천 서비스 (후보 선정)
│   └── balance_service.py           # 잔고 및 주문 서비스
├── api/
│   └── routes/
│       ├── auto_trading.py          # 자동매매 API 라우터
│       └── stock_recommendations.py  # 추천 API 라우터
├── utils/
│   └── scheduler.py                 # 스케줄러 (트리거)
├── schemas/
│   └── auto_trading.py              # 데이터 모델
└── core/
    └── config.py                     # 설정 관리
```

### 🔗 주요 함수 호출 체인

```
scheduler._run_auto_buy()
    └─> scheduler._execute_auto_buy()
        └─> auto_trading_service.get_buy_candidates()
            └─> stock_recommendation_service.get_combined_recommendations_with_technical_and_sentiment()
                ├─> Supabase: stock_analysis_results 조회
                ├─> Supabase: stock_recommendations 조회
                └─> Supabase: ticker_sentiment_analysis 조회
        └─> balance_service.get_overseas_balance()
        └─> balance_service.get_current_price()
        └─> auto_trading_service.calculate_buy_quantity()
            └─> balance_service.inquire_psamount()
        └─> balance_service.order_overseas_stock()
            └─> KIS API 호출
        └─> auto_trading_service._save_order_log()
            └─> Supabase: auto_trading_logs 저장
```

---

## 데이터 흐름 다이어그램

### 📊 데이터 흐름

```
┌─────────────────────────────────────────────────────────────┐
│                    데이터 소스 레이어                          │
├─────────────────────────────────────────────────────────────┤
│                                                               │
│  Supabase                                                    │
│  ├─ stock_analysis_results (AI 예측)                         │
│  │  └─ accuracy, rise_probability, predicted_price          │
│  │                                                           │
│  ├─ stock_recommendations (기술적 지표)                      │
│  │  └─ golden_cross, rsi, macd_buy_signal                   │
│  │                                                           │
│  ├─ ticker_sentiment_analysis (감정 분석)                    │
│  │  └─ average_sentiment_score, article_count                │
│  │                                                           │
│  └─ auto_trading_config (설정)                               │
│     └─ min_composite_score, max_stocks_to_buy, ...           │
│                                                               │
│  MongoDB                                                     │
│  ├─ users (사용자 설정)                                       │
│  │  └─ stocks[].use_leverage, leverage_ticker               │
│  │                                                           │
│  └─ daily_stock_data (주가 데이터)                           │
│     └─ 기술적 지표 계산용                                     │
│                                                               │
└───────────────────────┬─────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────────┐
│              추천 서비스 (통합 분석)                           │
│  get_combined_recommendations_with_technical_and_sentiment()│
│                                                               │
│  1. AI 예측 데이터 조회                                       │
│  2. 기술적 지표 데이터 조회                                   │
│  3. 감정 분석 데이터 조회                                     │
│  4. 데이터 통합 및 필터링                                     │
│  5. 종합 점수 계산                                            │
│     composite_score = (상승확률 × 0.3) +                      │
│                       (기술적지표 × 0.4) +                    │
│                       (감정점수 × 0.3)                        │
└───────────────────────┬─────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────────┐
│              자동매매 서비스 (후보 선정)                       │
│  get_buy_candidates()                                        │
│                                                               │
│  1. 종합 점수 필터링 (min_composite_score)                    │
│  2. 감정 점수 필터링 (min_sentiment_score, 선택적)            │
│  3. 최대 종목 수 제한 (max_stocks_to_buy)                     │
│  4. 종합 점수 기준 정렬                                       │
└───────────────────────┬─────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────────┐
│              스케줄러 (매수 실행)                              │
│  _execute_auto_buy()                                        │
│                                                               │
│  각 종목별 처리:                                             │
│  1. 레버리지 설정 확인 (MongoDB users)                        │
│  2. 거래소 코드 결정                                          │
│  3. 현재가 조회 (KIS API)                                     │
│  4. 매수 가능 수량 계산 (KIS API)                             │
│  5. 매수 주문 실행 (KIS API)                                  │
│  6. 주문 기록 저장 (Supabase)                                │
│  7. Slack 알림 전송                                          │
└─────────────────────────────────────────────────────────────┘
```

---

## 코딩 가이드라인

### ✅ 필수 체크리스트

#### 1. 에러 처리

```python
# ✅ 올바른 예시
try:
    result = api_call()
    if result.get("rt_cd") != "0":
        logger.error(f"API 오류: {result.get('msg1')}")
        return {"success": False, "error": result.get("msg1")}
except Exception as e:
    logger.error(f"예외 발생: {str(e)}", exc_info=True)
    return {"success": False, "error": str(e)}

# ❌ 잘못된 예시
result = api_call()  # 예외 처리 없음
data = result["output"]  # KeyError 가능
```

#### 2. 타입 변환

```python
# ✅ 올바른 예시
try:
    price = float(price_str or 0)
    if price <= 0:
        logger.warning("유효하지 않은 가격")
        continue
except (ValueError, TypeError):
    logger.error("가격 변환 실패")
    continue

# ❌ 잘못된 예시
price = float(price_str)  # None이면 에러
```

#### 3. None 체크

```python
# ✅ 올바른 예시
if result and result.get("output"):
    data = result["output"]
else:
    logger.warning("데이터 없음")
    return

# ❌ 잘못된 예시
data = result["output"]  # result가 None이면 에러
```

#### 4. 설정 기반 로직

```python
# ✅ 올바른 예시
config = self.get_auto_trading_config()
min_score = config.get("min_composite_score", 2.0)
if stock.get("composite_score", 0) < min_score:
    continue

# ❌ 잘못된 예시
if stock.get("composite_score", 0) < 2.0:  # 하드코딩
    continue
```

#### 5. 로깅

```python
# ✅ 올바른 예시
logger.info(f"[{function_name}] 매수 시작: {ticker}")
logger.error(f"[{function_name}] 매수 실패: {error}", exc_info=True)

# ❌ 잘못된 예시
print(f"매수 시작: {ticker}")  # 로거 사용 안 함
```

### 📝 코딩 패턴

#### 패턴 1: 설정 기반 필터링

```python
def filter_candidates(candidates: List[Dict], config: Dict) -> List[Dict]:
    """설정 기반 후보 필터링"""
    filtered = []
    
    for candidate in candidates:
        # 종합 점수 필터링
        if candidate.get("composite_score", 0) < config.get("min_composite_score", 2.0):
            continue
        
        # 감정 점수 필터링 (선택적)
        if config.get("use_sentiment", False):
            sentiment = candidate.get("sentiment_score")
            if sentiment is None or sentiment < config.get("min_sentiment_score", 0.15):
                continue
        
        filtered.append(candidate)
    
    return filtered
```

#### 패턴 2: 안전한 API 호출

```python
def safe_api_call(api_func, *args, **kwargs):
    """안전한 API 호출 래퍼"""
    max_retries = 3
    for attempt in range(max_retries):
        try:
            result = api_func(*args, **kwargs)
            
            # 응답 검증
            if result.get("rt_cd") != "0":
                error_msg = result.get("msg1", "알 수 없는 오류")
                logger.warning(f"API 오류 (시도 {attempt+1}/{max_retries}): {error_msg}")
                
                if attempt < max_retries - 1:
                    time.sleep(2 ** attempt)  # 지수 백오프
                    continue
                else:
                    return {"success": False, "error": error_msg}
            
            return {"success": True, "data": result}
            
        except Exception as e:
            logger.error(f"API 호출 예외 (시도 {attempt+1}/{max_retries}): {str(e)}")
            if attempt < max_retries - 1:
                time.sleep(2 ** attempt)
                continue
            else:
                return {"success": False, "error": str(e)}
    
    return {"success": False, "error": "최대 재시도 횟수 초과"}
```

#### 패턴 3: 대체 가격 사용

```python
def get_price_with_fallback(ticker: str, candidate: Dict) -> float:
    """현재가 조회 (대체 가격 포함)"""
    # 1차: 실시간 가격 조회
    price_result = get_current_price({"EXCD": "NAS", "SYMB": ticker})
    
    if price_result.get("rt_cd") == "0":
        output = price_result.get("output", {})
        last_price = float(output.get("last") or 0)
        
        if last_price > 0:
            return last_price
        
        # 2차: 전일 종가 사용
        base_price = float(output.get("base") or 0)
        if base_price > 0:
            logger.warning(f"{ticker} - 전일 종가 사용: {base_price}")
            return base_price
    
    # 3차: 추천 데이터의 가격 사용
    cached_price = float(candidate.get("last_price") or 0)
    if cached_price > 0:
        logger.warning(f"{ticker} - 추천 당시 가격 사용: {cached_price}")
        return cached_price
    
    predicted_price = float(candidate.get("predicted_price") or 0)
    if predicted_price > 0:
        logger.warning(f"{ticker} - AI 예측 가격 사용: {predicted_price}")
        return predicted_price
    
    raise ValueError(f"{ticker} - 유효한 가격을 찾을 수 없음")
```

---

## 설정 및 커스터마이징

### ⚙️ 자동매매 설정

**테이블**: `auto_trading_config` (Supabase)

**설정 항목**:

| 항목 | 타입 | 기본값 | 설명 |
|-----|------|-------|------|
| `enabled` | boolean | `false` | 자동매매 활성화 여부 |
| `min_composite_score` | float | `2.0` | 최소 종합 점수 |
| `max_stocks_to_buy` | int | `5` | 최대 매수 종목 수 |
| `max_amount_per_stock` | float | `10000.0` | 종목당 최대 매수 금액 (USD) |
| `stop_loss_percent` | float | `-7.0` | 손절 기준 (%) |
| `take_profit_percent` | float | `5.0` | 익절 기준 (%) |
| `use_sentiment` | boolean | `true` | 감정 분석 사용 여부 |
| `min_sentiment_score` | float | `0.15` | 최소 감정 점수 |
| `order_type` | string | `"00"` | 주문 구분 (00: 지정가) |

**설정 업데이트 방법**:

```python
# API를 통한 설정 업데이트
PUT /api/auto-trading/config
{
    "enabled": true,
    "min_composite_score": 2.5,
    "max_stocks_to_buy": 3,
    "max_amount_per_stock": 5000.0
}
```

### 🎯 매수 조건 커스터마이징

#### 1. 종합 점수 기준 변경

**파일**: `app/services/auto_trading_service.py`

```python
# 현재 코드
if stock.get("composite_score", 0) < config.get("min_composite_score", 2.0):
    continue

# 더 보수적으로 변경
if stock.get("composite_score", 0) < 2.5:  # 2.0 → 2.5
    continue

# 더 공격적으로 변경
if stock.get("composite_score", 0) < 1.5:  # 2.0 → 1.5
    continue
```

#### 2. 감정 점수 기준 변경

**파일**: `app/services/auto_trading_service.py`

```python
# 현재 코드
if sentiment_score is None or sentiment_score < config.get("min_sentiment_score", 0.15):
    continue

# 더 보수적으로 변경
if sentiment_score is None or sentiment_score < 0.25:  # 0.15 → 0.25
    continue

# 더 공격적으로 변경
if sentiment_score is None or sentiment_score < 0.10:  # 0.15 → 0.10
    continue
```

#### 3. 기술적 지표 조건 변경

**파일**: `app/services/stock_recommendation_service.py` → `get_combined_recommendations_with_technical_and_sentiment()`

```python
# 현재 코드
if sentiment_score is not None and sentiment_score >= 0.15:
    if sum(tech_conditions) >= 2:  # 기술적 지표 2개
        final_results.append(item)
else:
    if sum(tech_conditions) >= 3:  # 기술적 지표 3개
        final_results.append(item)

# 더 보수적으로 변경
if sentiment_score is not None and sentiment_score >= 0.25:
    if sum(tech_conditions) >= 3:  # 2개 → 3개
        final_results.append(item)
else:
    if sum(tech_conditions) >= 3:  # 유지
        final_results.append(item)

# 더 공격적으로 변경
if sentiment_score is not None and sentiment_score >= 0.10:
    if sum(tech_conditions) >= 1:  # 2개 → 1개
        final_results.append(item)
else:
    if sum(tech_conditions) >= 2:  # 3개 → 2개
        final_results.append(item)
```

---

## API 엔드포인트 정리

### 🔌 자동매매 API

#### 1. 설정 조회

```http
GET /api/auto-trading/config
```

**응답**:
```json
{
  "success": true,
  "config": {
    "enabled": true,
    "min_composite_score": 2.0,
    "max_stocks_to_buy": 5,
    "max_amount_per_stock": 10000.0,
    "stop_loss_percent": -7.0,
    "take_profit_percent": 5.0,
    "use_sentiment": true,
    "min_sentiment_score": 0.15,
    "order_type": "00"
  }
}
```

#### 2. 설정 업데이트

```http
PUT /api/auto-trading/config
Content-Type: application/json

{
  "enabled": true,
  "min_composite_score": 2.5,
  "max_stocks_to_buy": 3
}
```

#### 3. 매수 후보 조회

```http
GET /api/auto-trading/candidates/buy
```

**응답**:
```json
{
  "success": true,
  "message": "3개의 매수 추천 종목을 찾았습니다",
  "candidates": [
    {
      "ticker": "NVDA",
      "stock_name": "엔비디아",
      "composite_score": 2.8,
      "sentiment_score": 0.35,
      "accuracy": 85.2,
      "rise_probability": 8.5,
      "last_price": 450.00,
      "predicted_price": 485.00
    }
  ]
}
```

#### 4. 자동 매수 실행

```http
POST /api/auto-trading/execute/buy
Content-Type: application/json

{
  "dry_run": false
}
```

**응답**:
```json
{
  "success": true,
  "message": "2개 종목 주문 완료",
  "orders": [
    {
      "ticker": "NVDA",
      "stock_name": "엔비디아",
      "price": 450.00,
      "quantity": 10,
      "estimated_amount": 4500.00,
      "composite_score": 2.8,
      "status": "success"
    }
  ],
  "config": { ... }
}
```

#### 5. 자동매매 상태 조회

```http
GET /api/auto-trading/status
```

**응답**:
```json
{
  "success": true,
  "status": {
    "config": { ... },
    "holdings": {
      "count": 5,
      "total_value": 25000.0,
      "items": [ ... ]
    },
    "available_cash": 222.64,
    "candidates": {
      "buy": {
        "count": 3,
        "items": [ ... ]
      },
      "sell": {
        "count": 1,
        "items": [ ... ]
      }
    },
    "recent_orders": [ ... ]
  }
}
```

**응답 필드 설명**:
- `config`: 현재 자동매매 설정
- `holdings`: 보유 종목 정보
  - `count`: 보유 종목 수
  - `total_value`: 총 평가 금액 (USD)
  - `items`: 종목별 상세 정보
- `available_cash`: 현금 잔고 (USD) - 매수 가능한 현금 금액
- `candidates`: 매수/매도 후보
  - `buy`: 매수 추천 종목 목록
  - `sell`: 매도 대상 종목 목록
- `recent_orders`: 최근 7일간 주문 내역

### 🔌 추천 시스템 API

#### 1. 통합 추천 조회

```http
GET /api/recommended-stocks/with-technical-and-sentiment
```

#### 2. 통합 분석 생성

```http
POST /api/recommended-stocks/generate-complete-analysis
```

---

## 에러 처리 및 로깅

### 🚨 주요 에러 시나리오

#### 1. API 호출 실패

```python
# 에러 처리 예시
try:
    result = get_current_price(params)
    if result.get("rt_cd") != "0":
        error_msg = result.get("msg1", "알 수 없는 오류")
        logger.error(f"현재가 조회 실패: {error_msg}")
        # 대체 가격 사용
        current_price = candidate.get("last_price", 0)
except Exception as e:
    logger.error(f"현재가 조회 예외: {str(e)}", exc_info=True)
    continue  # 다음 종목으로
```

#### 2. 잔고 부족

```python
# 잔고 부족 처리
quantity_result = self.calculate_buy_quantity(...)
if not quantity_result.get("success") or quantity_result.get("quantity", 0) <= 0:
    logger.warning(f"{ticker} - 매수 가능 수량 없음 (잔고 부족)")
    skipped_no_cash += 1
    continue
```

#### 3. 주문 실패

```python
# 주문 실패 처리
order_result = order_overseas_stock(order_data)
if order_result.get("rt_cd") != "0":
    error_msg = order_result.get("msg1", "알 수 없는 오류")
    logger.error(f"{ticker} - 매수 주문 실패: {error_msg}")
    # Slack 알림 전송
    slack_notifier.send_buy_notification(..., success=False, error_message=error_msg)
    continue
```

### 📝 로깅 가이드

#### 로그 레벨

```python
# INFO: 일반적인 정보
logger.info(f"[{function_name}] 매수 시작: {ticker}")

# WARNING: 경고 (계속 진행 가능)
logger.warning(f"{ticker} - 잔고 부족으로 매수 건너뜀")

# ERROR: 오류 (처리 실패)
logger.error(f"{ticker} - 매수 주문 실패: {error_msg}", exc_info=True)

# DEBUG: 디버깅 정보
logger.debug(f"주문 데이터: {order_data}")
```

#### 로그 포맷

```python
# 함수명 포함
logger.info(f"[{function_name}] 작업 시작")

# 상세 정보 포함
logger.info(f"[{function_name}] {stock_name}({ticker}) 매수 성공: {quantity}주 @ ${price}")

# 에러 시 스택 트레이스 포함
logger.error(f"[{function_name}] 오류 발생: {str(e)}", exc_info=True)
```

---

## 실전 활용 예시

### 🎯 시나리오 1: 자동 매수 시스템 시작

```python
import requests

BASE_URL = "http://localhost:8000/api"

# 1. 자동매매 설정 확인
response = requests.get(f"{BASE_URL}/auto-trading/config")
config = response.json()["config"]

if not config.get("enabled"):
    # 2. 자동매매 활성화
    requests.put(f"{BASE_URL}/auto-trading/config", json={"enabled": True})

# 3. 매수 후보 확인
response = requests.get(f"{BASE_URL}/auto-trading/candidates/buy")
candidates = response.json()["candidates"]
print(f"매수 후보: {len(candidates)}개")

# 4. Dry run으로 테스트
response = requests.post(f"{BASE_URL}/auto-trading/execute/buy", json={"dry_run": True})
print(response.json())

# 5. 실제 매수 실행
response = requests.post(f"{BASE_URL}/auto-trading/execute/buy", json={"dry_run": False})
print(response.json())
```

### 🎯 시나리오 2: 보수적 매수 전략 설정

```python
# 보수적 설정 적용
config_update = {
    "enabled": True,
    "min_composite_score": 2.5,  # 2.0 → 2.5 (더 높은 기준)
    "max_stocks_to_buy": 3,      # 5 → 3 (더 적은 종목)
    "max_amount_per_stock": 5000.0,  # 10000 → 5000 (더 적은 금액)
    "use_sentiment": True,
    "min_sentiment_score": 0.25  # 0.15 → 0.25 (더 높은 감정 점수)
}

requests.put(f"{BASE_URL}/auto-trading/config", json=config_update)
```

### 🎯 시나리오 3: 공격적 매수 전략 설정

```python
# 공격적 설정 적용
config_update = {
    "enabled": True,
    "min_composite_score": 1.5,  # 2.0 → 1.5 (더 낮은 기준)
    "max_stocks_to_buy": 10,     # 5 → 10 (더 많은 종목)
    "max_amount_per_stock": 15000.0,  # 10000 → 15000 (더 많은 금액)
    "use_sentiment": True,
    "min_sentiment_score": 0.10  # 0.15 → 0.10 (더 낮은 감정 점수)
}

requests.put(f"{BASE_URL}/auto-trading/config", json=config_update)
```

### 🎯 시나리오 4: 매수 상태 모니터링

```python
import time
from datetime import datetime

def monitor_auto_trading():
    """자동매매 상태 모니터링"""
    while True:
        response = requests.get(f"{BASE_URL}/auto-trading/status")
        status = response.json()["status"]
        
        print(f"\n[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]")
        print(f"보유 종목: {status['holdings']['count']}개")
        print(f"총 평가 금액: ${status['holdings']['total_value']:,.2f}")
        print(f"매수 후보: {status['candidates']['buy']['count']}개")
        print(f"매도 후보: {status['candidates']['sell']['count']}개")
        
        time.sleep(60)  # 1분마다 확인

# 백그라운드에서 실행
import threading
thread = threading.Thread(target=monitor_auto_trading, daemon=True)
thread.start()
```

---

## 📚 참고 문서

- [매수 동작 가이드](./매수_동작_가이드.md) - 매수 시스템 상세 가이드
- [스케줄러 가이드](../system/스케줄러_가이드.md) - 스케줄러 전체 가이드
- [통합 추천 시스템 가이드](../recommendation/통합추천시스템_가이드.md) - 추천 시스템 가이드

---

## 🔄 최근 변경사항

### 2025-12-12
- 레버리지 설정 지원 추가 (MongoDB users 컬렉션)
- 여러 거래소에서 현재가 조회 시도 로직 추가
- 대체 가격 사용 로직 개선 (전일 종가, 추천 가격, 예측 가격)

### 2025-12-10
- 자동매매 서비스 분리 (`auto_trading_service.py`)
- 설정 기반 필터링 로직 개선
- API 엔드포인트 정리 및 문서화

---

**마지막 업데이트**: 2025-12-12

**작성자**: Cursor AI

**버전**: 1.0.0



