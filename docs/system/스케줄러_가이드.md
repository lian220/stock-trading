...
| 작업 | 실행 시간 (한국) | 작업 내용 |
|-----|-----------------|----------|
| **경제 데이터 수집** | 매일 06:05 | 경제 지표 및 주가 데이터 수집 |
| **경제 데이터 재수집** | 매일 23:00 | 최신 지표 반영을 위한 경제 데이터 재수집 |
| **Vertex AI 예측** | 매일 23:00 | Vertex AI를 사용한 주가 예측 작업 실행 |
| **병렬 분석** | 매일 23:15 | 기술적 지표 + 감정 분석 (병렬 실행) |
| **통합 분석** | 매일 23:45 | AI 예측 + 기술적 지표 + 감정 분석 통합 |
| **매수 스케줄러** | 매일 00:00 | 추천 종목 자동 매수 |
| **매도 스케줄러** | 5분마다 | 보유 종목 자동 매도 |
| **미체결 주문 정리** | 매일 06:30 | 장 마감 후 미체결 주문 실패 처리 |
...
06:05 - 경제 데이터 수집 (장 마감 후)
        ↓
23:00 - 경제 데이터 재수집 + Vertex AI 예측 시작
        ↓
23:15 - 병렬 분석 (기술적 지표 + 감정 분석)
        ↓
23:45 - 통합 분석 (AI 예측 + 기술적 지표 + 감정 분석)
        ↓
00:00 - 자동 매수 실행 (장 시작 직전)
        ↓
(5분마다) - 자동 매도 체크 (장 시간 중)
        ↓
06:30 - 미체결 주문 정리 (장 마감 후)
```

### 🔗 API 엔드포인트 동작 방식

#### `/economic/update` (경제 데이터 수집)
- **날짜 파라미터 없음**: 마지막 수집 날짜+1 ~ 오늘까지 수집
- **날짜 파라미터 있음**: `start_date` ~ `end_date` 범위로 수집

#### `/recommended-stocks/generate-technical-recommendations` (기술적 지표 분석)
- **날짜 파라미터 없음**: 최근 6개월 데이터로 계산, 오늘 날짜로 저장
- **날짜 파라미터 있음**: 해당 날짜 범위로 계산, `end_date`로 저장

#### `/recommended-stocks/analyze-news-sentiment` (감정 분석)
- **날짜 파라미터 없음**: 최근 3일 뉴스 수집, 오늘 날짜로 저장
- **날짜 파라미터 있음**: `start_date` 기준 뉴스 수집, `end_date`로 저장
...
```
1. 장 마감 확인 (뉴욕 시간 기준)
   └─ 16:00 ET 이후 또는 주말인지 확인
   └─ 장 마감 전이면 종료
   
2. 미체결 주문 조회
   └─ 어제부터 오늘까지의 pending/accepted 상태 주문 조회
   └─ trading_logs 컬렉션에서 조회
   
3. 각 주문 실패 처리
   ├─ 상태를 "failed"로 변경
   ├─ failed_at, failure_reason 필드 추가
   ├─ 개별 Slack 알림 전송
   └─ 로그 기록
   
4. 요약 Slack 알림
   └─ 총 미체결 주문 수, 실패 처리 완료 수
   └─ 실패 처리된 주문 목록 (최대 10개)
```
...
|-----|------|
| `pending` | 미체결 상태인 주문 |
| `accepted` | 주문 접수되었으나 체결되지 않은 주문 |

**처리 결과**: 모든 미체결 주문이 `failed` 상태로 변경됨

### 📝 로그 예시

```
2025-12-04 06:30:00 - 미체결 주문 정리 작업 시작
2025-12-04 06:30:01 - 미체결 주문 3개 발견
2025-12-04 06:30:02 - ✅ Apple Inc.(AAPL) 미체결 주문 실패 처리 완료
2025-12-04 06:30:03 - ✅ Microsoft Corp.(MSFT) 미체결 주문 실패 처리 완료
2025-12-04 06:30:04 - ✅ Google A(GOOGL) 미체결 주문 실패 처리 완료
2025-12-04 06:30:05 - 함수 실행 완료: 3개 주문 실패 처리
```
...
실패 처리된 주문:
  - Apple Inc.(AAPL): 10주 @ $150.00
  - Microsoft Corp.(MSFT): 5주 @ $380.00
  - Google A(GOOGL): 8주 @ $140.00
```
...
pending_orders = db.trading_logs.find({
...
```json
{
  "message": "자동 매도 스케줄러가 시작되었습니다. 5분마다 매도 대상을 확인합니다."
}
```
...
| **매도** | 5분마다 | 5분마다 | 장 시간(9:30-16:00)일 때만 |
